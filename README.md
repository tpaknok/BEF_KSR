
<!-- README.md is generated from README.Rmd. Please edit that file -->

# Overview

This repository contains all the scripts required to reproduce results
in the manuscript **Species overlap and phylogenetic relatedness result
in community statistical non-independence (and what to do about it)**.

# Code

Two scripts are provided, one for the simulation and another for the
empirical analyses.

| Scripts             | Description                             |
|---------------------|-----------------------------------------|
| updated_sim_spaMM.R | The script for the simulation analyses. |
| KSR_empirical.R     | The script for the empirical analyses.  |

# Data

Please install the [EcoCoMix
package](https://github.com/tpaknok/EcoCoMix), which contains the data
for the empirical analyses.

For the simulation, you can open sim500.RData, or you can simply run the
simulation script. Note that the latter will take a much longer time. It
took me \>24 hours to finish the simulation with my laptop (AMD Ryzen 7
6800H 3.20GHz, 16GB RAM)

# Figure

The folder contains all figures included in the manuscript.

# Table

The folder contains all tables included in the manuscript.

# Contact

Toby Tsang (<paknok.tsang@utoronto.ca>)
