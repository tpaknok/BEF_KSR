...
If you are interested in using EcoCoMix, please visit this
[repository](https://tpaknok.github.io/EcoCoMix/articles/Empirical_single.html)
...
Three scripts are provided, two for the simulations and one for the
empirical analyses.
...
required to reproduce the results have also been provided in the Data
folder.
...
KSR_MLtree.tre (or data(KSR_MLtree)) is a phylogeny tree containing the
14 species used in the experiment.
...
Alternatively, you can load the results (sim500.RData or simSupp.Rdata)
directly.

# Figure

The folder contains all result figures included in the manuscript:
p_coef (Figure S1), p_KSR (Figure 4), p_KSR_all (Figure S2), and p_sim
(Figure 3). The code used to reproduce these figures is provided in the
simulation and empirical analysis scripts.
...
The code was run using RStudio 2025.09.1 Build 401 and R version 4.5.1
on Windows 11 x64 (build 26200). Please see sessionInfo.txt for all R
packages used.
