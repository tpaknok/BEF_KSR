...
# Overview
This repository contains all the scripts required to reproduce results in the manuscript **Species overlap and phylogenetic relatedness result in community statistical non-independence (and what to do about it)**. If you are interested in using EcoCoMix, please visit the [webiste](https://tpaknok.github.io/EcoCoMix/articles/Empirical_single.html)
...
# Code
Two scripts are provided, one for the simulation and another for the empirical analyses.
...
# Data - Overview
Please install the [EcoCoMix package](https://github.com/tpaknok/EcoCoMix), which contains the necessary functions and also data for the empirical analyses. The data have also been provided in the Data folder.
...
## Simulation
For the simulation, you can simply run the simulation scripts. However, this will take a long time. It took me >24 hours to finish the simulation with my laptop (AMD Ryzen 7 6800H 3.20GHz, 16GB RAM).

IF you don't want ot wait, you can load the results (sim500.RData or simSupp.Rdata) and obtain summary statistics.

# Figure
The folder contains all result figures included in the manuscript: p_coef(Figure S1), p_KSR (Figure 4), p_KSR_all(Figure S2), p_sim (Figure 3). Note that the code to reproduce the figures is in the simulation and empirical analysis scripts.
